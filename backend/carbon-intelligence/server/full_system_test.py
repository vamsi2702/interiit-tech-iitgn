#!/usr/bin/env python3
"""
Full System Integration Test for Carbon Intelligence Platform
...
Prerequisites:
    ...
    - Wait ~30s for Debezium connector initialization
"""
...
KAFKA_BROKER = "localhost:29092"  # External port for testing
KAFKA_INTERNAL_BROKER = "kafka:9092"  # Internal Docker network
DEBEZIUM_CONNECT_URL = "http://localhost:8083"
REDIS_HOST = "localhost"  # or "redis" if inside Docker
REDIS_PORT = 6379
GRPC_TARGET = "localhost:50051"

# Test data identifiers
TEST_NEWS_SOURCE = "system-test"
TEST_VERRA_ID = "test-verra-001"
TEST_CARBONMARK_ID = "test-carbonmark-001"
...
def test_database_schema():
    ...
    required_tables = {
        "verra": ["id", "name", "status", "country", "updated_at"],
        "carbonmark": ["id", "project_id", "vintage", "amount", "last_update"],
...
def insert_test_data() -> Dict[str, str]:
    ...
    try:
        ...
        # Insert test news
        news_guid = f"test-news-{uuid.uuid4()}"
        cur.execute("""
        ...
        """, (
            news_guid,
            "Test Carbon Market Update",
            "http://test.example.com/news",
            TEST_NEWS_SOURCE,
            "This is a test news article for system testing"
        ))
        test_guids['news'] = news_guid
        log_info(f"Inserted test news: {news_guid}")
        
        # Insert test verra project
        verra_id = f"test-verra-{uuid.uuid4()}"
        cur.execute("""
            INSERT INTO verra (id, name, status, country, updated_at)
            VALUES (%s, %s, %s, %s, NOW())
        ...
        test_guids['verra'] = verra_id
        log_info(f"Inserted test verra: {verra_id}")
        
        # Insert test carbonmark
        carbonmark_id = f"test-carbonmark-{uuid.uuid4()}"
        cur.execute("""
            INSERT INTO carbonmark (id, project_id, vintage, amount, last_update)
            VALUES (%s, %s, %s, %s, NOW())
            ON CONFLICT (id) DO UPDATE SET last_update = NOW()
        """, (
            carbonmark_id,
            verra_id,  # Link to verra project
            2024,
            1000.50
        ))
        test_guids['carbonmark'] = carbonmark_id
        log_info(f"Inserted test carbonmark: {carbonmark_id}")
        
        # Insert/update test finance data
        cur.execute("""
...
def test_debezium_connector():
    ...
    try:
        ...
        if "postgres-connector" not in connectors:
            log_warning("postgres-connector not found, attempting to create...")
            return create_debezium_connector()
...
def create_debezium_connector():
    """Create Debezium connector if it doesn't exist"""
    connector_config = {
    ...
    try:
        ...
        log_success("Created Debezium connector")
        time.sleep(5)  # Wait for initialization
        return True
        
    except Exception as e:
        log_error(f"Failed to create connector: {e}")
        return False
...
def test_grpc_cache_integration(test_guids: Dict[str, str]):
    """Test 15: Verify gRPC responses are cached"""
    log_test("gRPC + Redis Cache Integration")
    ...
    try:
        ...
        # Clear cache
        keys_before = r.keys("*")
        log_info(f"Cache keys before: {len(keys_before)}")
        
        # Make gRPC call
        response = stub.GetNews(pb2.NewsQuery(source=TEST_NEWS_SOURCE))
        time.sleep(0.5)  # Allow cache write
        
        # Check cache was populated
        keys_after = r.keys("*")
        log_info(f"Cache keys after: {len(keys_after)}")
        
        expected_key = f"news:{TEST_NEWS_SOURCE}"
        if expected_key in keys_after or any(TEST_NEWS_SOURCE in k for k in keys_after):
            log_success("Response was cached")
            channel.close()
            return True
        else:
            log_warning("Response not found in cache")
            log_info(f"Looking for key matching: {expected_key}")
            log_info(f"Available keys: {keys_after[:5]}")
            channel.close()
            return False
            
    except Exception as e:
        log_error(f"Cache integration test failed: {e}")
        return False
...
def test_end_to_end_flow(test_guids: Dict[str, str]):
    """Test 16: Complete data flow verification"""
    log_test("End-to-End Data Flow")
    
    if not test_guids or not pb2 or not pb2_grpc:
        log_warning("Skipping E2E test (missing dependencies)")
        return False
    
    try:
        # Step 1: Verify data in Postgres
        conn = get_postgres_connection()
        cur = conn.cursor()
        cur.execute("SELECT guid FROM news WHERE guid = %s", (test_guids['news'],))
        pg_result = cur.fetchone()
        conn.close()
        
        if not pg_result:
            log_error("Test data not found in Postgres")
            return False
        log_info("✓ Data in PostgreSQL")
        
        # Step 2: Check if it reached gRPC (via Kafka -> Pathway)
        channel = grpc.insecure_channel(GRPC_TARGET)
        stub = pb2_grpc.CarbonServiceStub(channel)
        ...
        response = stub.GetNews(pb2.NewsQuery(source=TEST_NEWS_SOURCE))
        guids_in_response = [item.guid for item in response.items]
        
        if test_guids['news'] in guids_in_response:
            log_info("✓ Data in gRPC response")
        else:
            log_warning("Test data not yet in gRPC response (CDC delay)")
        
        channel.close()
        
        # Step 3: Verify caching
        r = redis.Redis(host=REDIS_HOST, port=REDIS_PORT, decode_responses=True)
        cache_keys = r.keys("*news*")
        
        if cache_keys:
            log_info(f"✓ Data cached ({len(cache_keys)} keys)")
        
        log_success("End-to-end flow validated")
        return True
        
    except Exception as e:
        log_error(f"E2E test failed: {e}")
        return False
...
def run_all_tests():
    ...
    print("\n" + "=" * 70)
    print("🧪 CARBON INTELLIGENCE PLATFORM - FULL SYSTEM TEST")
    print("=" * 70)
    ...
    if grpc_ready:
        ...
        results.append(("Cache Integration", test_grpc_cache_integration(test_guids)))
    else:
        log_warning("gRPC server not ready, skipping gRPC tests")
        results.append(("gRPC Tests", False))
    
    # Phase 5: Integration
    print(f"\n{YELLOW}PHASE 5: Integration{RESET}")
    results.append(("End-to-End Flow", test_end_to_end_flow(test_guids)))
