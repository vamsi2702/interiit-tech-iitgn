          ...
          <div className="flex-1 max-w-2xl relative group">
            <form onSubmit={handleSearch} className="relative">
              ...
              {showSuggestions && suggestions.length > 0 && (
                <div className="absolute top-full mt-2 w-full bg-slate-800/98 backdrop-blur-2xl border border-green-500/40 rounded-xl shadow-2xl overflow-hidden animate-scaleIn">
                  <div className="absolute inset-0 bg-gradient-to-b from-green-500/5 to-transparent pointer-events-none"></div>
                  <div className="relative p-3 border-b border-green-500/30 flex items-center gap-2 bg-slate-900/40">
                    <Sparkles className="w-4 h-4 text-green-400 animate-pulse" />
                    <span className="text-xs text-green-400 font-semibold uppercase tracking-wider">RAG-Powered Results</span>
                  </div>
                  <div className="max-h-96 overflow-y-auto">
                    {suggestions.map((item, idx) => (
                      <button
                        ...
                        className="w-full text-left px-4 py-3 hover:bg-gradient-to-r hover:from-green-500/10 hover:to-emerald-500/10 transition-all duration-300 border-b border-slate-700/40 last:border-b-0 group animate-slideInLeft relative overflow-hidden"
                      >
                        ...
                        <div className="flex items-start justify-between relative">
                          <div className="flex-1">
                            <div className="flex items-center gap-2">
                              <span className="font-semibold text-slate-200 group-hover:text-green-300 transition-colors duration-300">{item.name}</span>
                              <span className="text-xs px-2 py-0.5 bg-green-500/20 text-green-300 rounded-md border border-green-500/40 group-hover:bg-green-500/30 transition-colors duration-300">{item.id}</span>
                            </div>
